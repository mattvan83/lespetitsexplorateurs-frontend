...
export default function Organizers(organizer) {
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("OrganizerProfile", { organizerDetailed })
        }
        style={styles.button}
        activeOpacity={0.8}
      >
        {organizer.imgUrl && (
          <Image
            source={{ uri: organizer.imgUrl }}
            style={styles.organizerImgProfile}
          />
        )}
        {organizer.imgUrl === "" && (
          <Text style={styles.initiale}>{organizer.name.slice(0, 1)}</Text>
        )}
      </TouchableOpacity>
